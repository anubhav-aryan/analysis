...
class DetailedCohortAnalysis:
    def __init__(self, file_path: str, cohort_name: str, folder_name: str):
        """Initialize the analyzer for any cohort with detailed 5-PNG analysis"""
    ...
    def create_overall_performance_viz(self):
        """Create overall performance visualization (PNG 1)"""
        print(f"📊 Creating {self.cohort_name} Overall Performance Visualization...")
        ...
        fig.suptitle(f'{self.cohort_name} Cohort: Overall Performance Analysis', fontsize=16, fontweight='bold')
        ...
        bars1 = ax2.bar(x - width/2, cohort_values, width, label=f'{self.cohort_name}', color=colors['primary'], alpha=0.7)
        bars2 = ax2.bar(x + width/2, benchmark_values, width, label='Benchmark', color='orange', alpha=0.7)
        
        ax2.set_title(f'Absolute Values: {self.cohort_name} vs Benchmark', fontweight='bold')
        ...
        bar_colors = [colors['secondary'] if ratio > 1 else 'lightcoral' for ratio in ratios]
        
        bars = ax3.bar(metrics, ratios, color=bar_colors, alpha=0.7)
        ax3.set_title(f'Performance Ratio ({self.cohort_name} / Benchmark)', fontweight='bold')
        ...
        ax4.text(0.1, 0.9, summary_text, transform=ax4.transAxes, fontsize=11,
                ...
                bbox=dict(boxstyle='round', facecolor=colors['primary'], alpha=0.5))
        
        plt.tight_layout()
        plt.savefig(f'{self.folder_name}/01_overall_performance.png', dpi=300, bbox_inches='tight')
        print(f"📊 {self.cohort_name} overall performance visualization saved")
    
    def create_spotlight_analysis_viz(self):
        """Create Spotlight analysis visualization (PNG 2)"""
        print(f"🔍 Creating {self.cohort_name} Spotlight Analysis Visualization...")
        ...
        if not spotlight_data:
            print(f"⚠️ No Spotlight data available for {self.cohort_name}")
            return
        
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle(f'{self.cohort_name} Cohort: Spotlight Feature Analysis', fontsize=16, fontweight='bold')
        ...
        bars1 = ax1.bar(x - width/2, cohort_values, width, label=f'{self.cohort_name}', color=colors['primary'], alpha=0.7)
        bars2 = ax1.bar(x + width/2, benchmark_values, width, label='Benchmark', color='lightblue', alpha=0.7)
        
        ax1.set_title('Spotlight Metrics vs Benchmark', fontweight='bold')
        ...
        bar_colors = [colors['secondary'] if pct > 0 else 'lightcoral' for pct in performance_pcts]
        
        bars = ax2.barh(metrics, performance_pcts, color=bar_colors, alpha=0.7)
        ...
        if efficiency_metrics:
            ...
            bars1 = ax3.bar(x - width/2, eff_cohort, width, label=f'{self.cohort_name}', color='gold', alpha=0.7)
        ...
        if adoption_metrics:
            ...
            bars1 = ax4.bar(x - width/2, adopt_cohort, width, label=f'{self.cohort_name}', color='lightgreen', alpha=0.7)
            bars2 = ax4.bar(x + width/2, adopt_benchmark, width, label='Benchmark', color='lightcoral', alpha=0.7)
        ...
        plt.tight_layout()
        plt.savefig(f'{self.folder_name}/02_spotlight_analysis.png', dpi=300, bbox_inches='tight')
        print(f"🔍 {self.cohort_name} spotlight analysis visualization saved")
    
    def create_dm_analysis_viz(self):
        """Create DM Dashboard analysis visualization (PNG 3)"""
        print(f"💬 Creating {self.cohort_name} DM Analysis Visualization...")
        ...
        if not dm_data:
            print(f"⚠️ No DM data available for {self.cohort_name}")
            return
        
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle(f'{self.cohort_name} Cohort: DM Dashboard Analysis', fontsize=16, fontweight='bold')
        ...
        if engagement_metrics:
            ...
            bars1 = ax1.bar(x - width/2, eng_cohort, width, label=f'{self.cohort_name}', color=colors['primary'], alpha=0.7)
        ...
        if adoption_metrics:
            ...
            bar_colors = [colors['secondary'] if pct > 0 else 'lightcoral' for pct in adopt_performance]
            
            bars = ax2.barh(adopt_metrics, adopt_performance, color=bar_colors, alpha=0.7)
        ...
        if usage_metrics:
            ...
            bar_colors = [colors['accent'] if ratio > 1 else 'lightcoral' for ratio in usage_ratios]
            
            bars = ax3.bar(usage_names, usage_ratios, color=bar_colors, alpha=0.7)
            ax3.set_title('Usage Pattern Ratios', fontweight='bold')
            ax3.set_ylabel(f'Ratio ({self.cohort_name} / Benchmark)')
            ax3.axhline(y=1, color='black', linestyle='-', alpha=0.3)
            ax3.tick_params(axis='x', rotation=45)
            plt.setp(ax3.get_xticklabels(), ha='right')
            ax3.grid(True, alpha=0.3)
        ...
        # Create DM summary
        summary_text = f"{self.cohort_name.upper()} DM PERFORMANCE\n\n"
        ...
        plt.tight_layout()
        plt.savefig(f'{self.folder_name}/03_dm_analysis.png', dpi=300, bbox_inches='tight')
        print(f"💬 {self.cohort_name} DM analysis visualization saved")
    
    def create_payment_analysis_viz(self):
        """Create Payment analysis visualization (PNG 4)"""
        print(f"💳 Creating {self.cohort_name} Payment Analysis Visualization...")
        ...
        if not payment_data:
            print(f"⚠️ No Payment data available for {self.cohort_name}")
            return
        
        fig, axes = plt.subplots(2, 2, figsize=(16, 12))
        fig.suptitle(f'{self.cohort_name} Cohort: Payment Performance Analysis', fontsize=16, fontweight='bold')
        ...
        ax1.set_title('Payment Metrics vs Benchmark', fontweight='bold')
        ax1.set_ylabel('Values')
        ax1.set_xticks(x)
        ax1.set_xticklabels(metrics, rotation=45, ha='right')
        ax1.legend()
        ...
        if transaction_metrics:
            ...
            ax3.plot(angles, trans_cohort, 'o-', linewidth=2, label=f'{self.cohort_name}', color=colors['accent'])
            ax3.fill(angles, trans_cohort, alpha=0.25, color=colors['primary'])
        ...
        if efficiency_metrics:
            ...
            bar_colors = ['lightcoral' if ratio > 1 else colors['accent'] for ratio in eff_ratios]  # For time metrics, lower is better
            
            bars = ax4.bar(eff_names, eff_ratios, color=bar_colors, alpha=0.7)
            ax4.set_title('Payment Efficiency Ratios', fontweight='bold')
            ax4.set_ylabel(f'Ratio ({self.cohort_name} / Benchmark)')
        ...
        plt.tight_layout()
        plt.savefig(f'{self.folder_name}/04_payment_analysis.png', dpi=300, bbox_inches='tight')
        print(f"💳 {self.cohort_name} payment analysis visualization saved")
    
    def create_comprehensive_dashboard(self):
        """Create comprehensive dashboard (PNG 5)"""
        ...
        if not all_performance:
            print(f"⚠️ No performance data available for {self.cohort_name} dashboard")
            return
        ...
        plt.tight_layout()
        plt.savefig(f'{self.folder_name}/05_comprehensive_dashboard.png', dpi=300, bbox_inches='tight')
...
def generate_detailed_cohort_analysis(cohort_name: str, folder_name: str):
    """Generate complete 5-PNG analysis for a specific cohort"""
    print(f"\n{'='*60}")
    print(f"🚀 STARTING DETAILED {cohort_name.upper()} COHORT ANALYSIS")
    print(f"{'='*60}")
    
    # Initialize analyzer
    analyzer = DetailedCohortAnalysis('Cohort Wise Analysis Fam 2.0 - Sheet1.csv', cohort_name, folder_name)
    ...
    # Create all 5 visualizations (same as 18+ and 18-)
    print(f"\n📊 Creating detailed visualizations for {cohort_name}...")
    analyzer.create_overall_performance_viz()      # PNG 1
    analyzer.create_spotlight_analysis_viz()       # PNG 2
    analyzer.create_dm_analysis_viz()              # PNG 3
    analyzer.create_payment_analysis_viz()         # PNG 4
    analyzer.create_comprehensive_dashboard()      # PNG 5
    
    # Generate business report
    avg_performance, positive_count, total_count = analyzer.generate_business_report()
    
    print(f"\n✅ {cohort_name} Detailed Analysis Complete!")
    print(f"📁 Generated 5 PNG files + business report in '{folder_name}' folder")
...
def main():
    """Main function to generate detailed analyses for all cohorts"""
    print("🚀 STARTING DETAILED 5-PNG ANALYSIS FOR ALL COHORTS")
    print("="*80)
    ...
    for cohort_name, folder_name in cohorts:
        try:
            result = generate_detailed_cohort_analysis(cohort_name, folder_name)
            if result:
    ...
    print(f"\n{'='*80}")
    print("📊 FINAL SUMMARY REPORT - ALL DETAILED ANALYSES")
    print(f"{'='*80}")
    ...
    if results:
        ...
        for i, result in enumerate(results, 1):
            ...
            print(f"{i:2d}. {status} {result['cohort_name']:15} | {result['avg_performance']:+6.1f}% | {result['success_rate']:5.1f}% success")
        ...
        print(f"\n📊 OVERALL STATISTICS:")
        print(f"   • Average Performance: {avg_performance_all:.1f}%")
        print(f"   • Average Success Rate: {avg_success_rate:.1f}%")
        print(f"   • Total Cohorts Analyzed: {len(results)}")
    
    print("\n✅ ALL DETAILED COHORT ANALYSES COMPLETED!")
    print("📁 Each cohort folder now contains:")
    ...
    print("   📋 [cohort]_business_report.txt")
